...
const generateToken = async (context, UserData ) => {
  ...
  try {
    ...
    data = UserData;
    data["token"] = jwt.sign({ userId: UserData.ID }, privateKey, { algorithm: 'RS256', expiresIn: "30d" })
    data["refreshToken"] = jwt.sign({ userId: UserData.ID }, privateKey, { algorithm: 'RS256', expiresIn: "150d" })
    return await data;
  } catch (e) {
...
const verifyToken = async (context ) => {
  ...
  try {
    ...
    if( Authorization ) {
      ...
      const { userId } = jwt.verify(token, publicKey, { algorithm: 'RS256' , expiresIn: "30d"  })
      data["UserID"] = userId
      return data
    } else {
  ...
  const regenerateToken = async( context, refresh ) => {
    ...
    try {
        var privateKey = fs.readFileSync("./news/schema/middleware/new.key");
        data["token"] = jwt.sign({ userId: context.UserID }, privateKey, { algorithm: 'RS256', expiresIn: "30d" })
  ...
  const regenerateCreativeToken = async( context, refresh ) => {
    ...
    try {
        ...
        data["CreativeToken"] = jwt.sign({ userId: context.UserID }, privateKey, { algorithm: 'RS256', expiresIn: "30d" })
...
module.exports = { generateToken, verifyToken,regenerateToken ,regenerateCreativeToken};
