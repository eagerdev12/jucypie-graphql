...
const ProfilePictureUpdate = {
  ...
  resolve: async (parent, args, context) => {
    ...
    var AwsUrl = await UploadBase64OnS3(
      args.ProfileImage,
      AWSCredentails.AWS_USER_IMG_PATH
    );
    var Message = {};
...
const UserSignUp = {
  ...
  args: {
    ...
    SignUpMethod: {
      type: GraphQLString,
      description:
        'enum fileds with value allowed : ["Site","Facebook", "Google", "Mobile"]',
    },
    ...
    Gender: {
      type: GraphQLString,
      description:
        'enum fileds with value allowed : ["Male","Female", "Other"]',
    },
  ...
  async resolve(parent, args, context) {
    ...
    if (
      typeof args.SignUpMethod != "undefined" &&
      args.SignUpMethod == "Site" &&
...
const ForgotPassword = {
  ...
  resolve(parent, args) {
    return Users.findOne({ Email: args.Email, isVerified: 1 }).then(
      (isEmail) => {
        var Message = {};
        if (isEmail != null && isEmail.Email) {
          sendSetPassworkLink(args);
          Message["Message"] =
            "We sent the link on " + args.Email + ". Please check your inbox.";
...
const UpdateUser = {
  ...
  resolve: async (parent, params, context) => {
    ...
    if (typeof params.Avatar != "undefined")
      params.Avatar = await UploadBase64OnS3(
        params.Avatar,
        AWSCredentails.AWS_USER_IMG_PATH
      );

    return await Users.findOneAndUpdate({ ID: params.ID }, params, {
      new: true,
