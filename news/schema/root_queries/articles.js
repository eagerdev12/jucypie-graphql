...
const ArticleBySlug = {
  ...
  resolve: async (parent, args, context) => {
    ...
    if (lor[0].ArticleScope == 2 && args.UserID != lor[0].AuthorID)
      lor = await calculateSubscribeContent(args, lor);
    ...
    if (typeof lor[0].SubTitle != "undefined") {
      if (
        typeof lor[0].Description != "undefined" &&
        lor[0].Description != null &&
        lor[0].Description != ""
      ) {
        lor[0].SubTitle = lor[0].Description.replace(/(<([^>]+)>)/gi, "");
        lor[0].SubTitle = lor[0].SubTitle.substring(0, SubTitleMaxLen) + "....";
      }
    }
    return await lor;
  },
...
const DashboardKeywordSerach = {
  ...
  resolve: async (parent, args, context) => {
    ...
    return Articles.find({
      Title: { $regex: args.Keywords, $options: "i" },
      Status: ArticleStatusConst.Approved,
    });
  },
};
    ...
    if (args.UserID == undefined) {
      ...
      return BlockAuthor.find(
        ...
        if (isAuthor.length == 0) {
          ...
          return Articles.find({
            AuthorID: { $nin: isAuthor.map((ID) => ID.AuthorID) },
            Status: ArticleStatusConst.Approved,
            isPublish: true,
          })
            .sort({ ViewCount: -1 })
...
const TrendingArticleList = {
  ...
  resolve: async (parent, args, context) => {
    ...
    if (args.UserID == undefined || args.UserID == 0)
      return await getAllTrendingArticles(
        args,
        args.limit,
        args.offset,
        args.UserID,
...
async function getAllTrendingArticles(args, limit, offset, ArgsUserID, query) {
  ...
  var IntrestData = [];
  if (ArgsUserID != 0)
    var IntrestData = await getUserIntrestData(
      args,
      ArgsUserID,
      sortOptions,
      offset,
  ...
  return FinalData.map((title) => {
    ...
    if (
      ...
      title.Title != null &&
      title.Title.length > TitleMaxLen
    )
    ...
    if (typeof title.SubTitle != "undefined") {
      if (
        typeof title.Description != "undefined" &&
        title.Description != null &&
        title.Description != ""
      ) {
        title.SubTitle = title.Description.replace(/(<([^>]+)>)/gi, "");
...
async function getUserIntrestData(
  args,
  ArgsUserID,
  sortOptions,
  offset,
  ...
  let UserData = await Users.findOne(
    { ID: ArgsUserID },
    { _id: false, SubCategories: true, ParentCategories: true }
  );
  // console.log(UserData,"UserData");
  if (UserData != null && UserData.ParentCategories.length > 0) {
    // console.log("in user data if");
    ...
    if (
      (filterInrestParent != null &&
        filterInrestSub != null &&
        filterInrestSub.length > 0) ||
      filterInrestParent.length > 0
    ) {
      // console.log("in leng if");
            ...
            $or: [
              { "Categories.Name": { $in: filterInrestParent } },
              {
                "Categories.SubCategories": {
                  $elemMatch: { Name: { $in: filterInrestSub } },
                },
              },
            ],
          },
...
async function IntrestCalculation(
  IntrestData,
  limit,
  offset,
  args,
  ...
  if (
    typeof IntrestData != "undefined" &&
    IntrestData != null &&
    IntrestData.length > 0
  ) {
    // console.log("in ifff");
    if (limit > IntrestData.length) {
      // if usr categories data finish but have limit then fetch articles
      limit = limit - IntrestData.length;
      Data = await Articles.find({ ID: { $nin: args.ArticleIds }, query })
        .sort(sortOptions)
        .skip(offset)
        .limit(limit);
    ...
    // query = query.Status
    delete query.ID;
    Data = Articles.find({ $and: [{ ID: { $nin: args.ArticleIds } }, query] })
      .sort(sortOptions)
      .skip(offset)
      .limit(limit);
    ...
    try {
      async.eachSeries(
        ...
        async (data, callback) => {
          ...
          return Promise.all([
            ArticleBookmarks.countDocuments(cquery),
            FollowAuthor.countDocuments(cauthr),
            ArticleRatings.countDocuments(cquery),
          ]).then(function (values) {
            data.isBookmark = values[0] >= 1 ? true : false;
            data.isFollowed = values[1] >= 1 ? true : false;
            data.isClicked = values[2] >= 1 ? true : false;
          });
        },
        async (err) => {
          if (err) reject(err);
...
function getAllPopularArticles() {
  return Articles.find({ Status: ArticleStatusConst.Approved, isPublish: true })
    .sort({ ViewCount: -1 })
    .limit(10);
}
...
const GetAdminArticleList = {
  ...
  resolve: async (parent, args, context) => {
    ...
    if (id.UserID) args.UserID = id.UserID;
    return Users.find({
      RoleID: { $in: [RoleObject.admin, RoleObject.moderator] },
      ID: args.UserID,
      Status: 1,
    }).then((isAdmin) => {
      if (isAdmin.length)
        return Articles.find({ Status: { $ne: 0 }, isPublish: 1 });
      else return [];
    });
  },
};
...
function getAllArticles() {
  return Articles.find({ Status: 1, isPublish: true });
}

const GetFreeArticles = {
  type: new GraphQLList(ArticleType),
  args: {
    AuthorID: { type: GraphQLInt },
    UserID: { type: GraphQLInt },
  ...
  resolve: async (parent, args, context) => {
    ...
    if (
      ...
      AuthorDetails = await Users.findOne({
        Status: 1,
...
module.exports = {
  ...
  GetAdminArticleList,
  PopularArticleList,
  DashboardKeywordSerach,
  GetParserArticleList,
  ArticleBySlug,
  ArticleByID,
  Article,
  ArticlesAll,
